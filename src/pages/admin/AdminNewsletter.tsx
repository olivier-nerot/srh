...
const AdminNewsletter: React.FC = () => {
  ...
  const [viewingNewsletter, setViewingNewsletter] =
    ...
    try {
      ...
      if (data.success) {
        ...
        if (selectedPublications.length === 0) {
          const allPublicationIds = data.publications.map(
        ...
        setMessage({
          type: "error",
          text: data.error || "Erreur lors du chargement des données",
        });
      }
    } catch (error) {
    ...
    try {
      ...
      if (delta.ops) {
        const text = delta.ops
          .reduce((acc: string, op: Record<string, unknown>) => {
  ...
  const handlePreview = async () => {
    if (!title.trim() || !hasValidContent()) {
      setMessage({
        type: "error",
        text: "Veuillez renseigner un titre et un contenu",
      });
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        ...
        setMessage({
          type: "error",
          text: data.error || "Erreur lors de la génération de l'aperçu",
        });
      }
    } catch (error) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setMessage({
          type: "error",
          text: data.error || "Erreur lors de la mise en file d'attente",
        });
      ...
      setMessage({
        type: "error",
        text: "Erreur lors de la mise en file d'attente",
      });
      setSendProgress(null);
    } finally {
      setIsSending(false);
  ...
  const handleDeleteNewsletter = async (id: number) => {
    ...
    try {
      ...
      if (data.success) {
        setMessage({
          type: "success",
          text: "Newsletter supprimée avec succès",
        });
        loadNewsletterData();
      } else {
        setMessage({
  ...
  const handleSaveDraft = async () => {
    // Read content directly from Quill to avoid stale closure issues
    let currentContent = content;
    if (quillRef && typeof quillRef === "object" && "getContents" in quillRef) {
      const quill = quillRef as { getContents: () => unknown };
      currentContent = JSON.stringify(quill.getContents());
    }
    ...
    try {
      ...
      setMessage({
        type: "error",
        text: "Erreur lors de la sauvegarde du brouillon",
      });
    } finally {
  ...
  const handleSendDraft = async (draftId: number) => {
    ...
    try {
      ...
      setMessage({
        type: "error",
        text: "Erreur lors de l'envoi du brouillon",
      });
    } finally {
      setIsSending(false);
    }
            ...
            <div className="flex items-start">
              ...
              <div className="flex-1">
                ...
                <div className="text-sm text-yellow-800 space-y-1">
                  <p>
                    <strong>
                      Aucun email ne sera envoyé aux membres réels.
                    </strong>
                  </p>
                  <p>
                    • Tous les emails seront redirigés vers :{" "}
                  ...
                  <p>
                    ...
                    <span className="font-semibold">
                      {debugMode.limit} emails maximum
                    </span>
                  </p>
                  <p>
                    • Les sujets des emails incluront le préfixe{" "}
                    <span className="font-mono bg-yellow-100 px-2 py-0.5 rounded">
                      [DEBUG]
                    </span>
                  </p>
                ...
                <div className="mt-3 pt-3 border-t border-yellow-300 text-xs text-yellow-700">
                  Pour désactiver : Mettre{" "}
                  <span className="font-mono">NEWSLETTER_DEBUG_MODE=false</span>{" "}
            ...
            <div className="flex items-start">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center justify-between text-xs text-blue-700">
                  <span>
                    {queueStatus.sentCount} / {queueStatus.totalRecipients}{" "}
                    emails envoyés
                ...
                <div className="p-6 space-y-6">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 pt-4 border-t border-gray-200">
                    ...
                    <button
                      onClick={handleSend}
                      disabled={
                        isSending || !title.trim() || !hasValidContent()
                      }
              ...
              <div className="bg-white rounded-lg shadow-sm border">
                ...
                {selectedPublications.length > 0 && (
                  <div className="px-4 py-3 bg-gray-50 border-t border-gray-200">
                    <p className="text-sm text-gray-600">
                      {selectedPublications.length} publication(s)
                      sélectionnée(s)
                    </p>
                  </div>
                )}
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-4 text-sm text-gray-600 mb-3">
                            ...
                            <div className="flex items-center">
                              ...
                              {newsletter.totalRecipients} envoyés
                            </div>
                          ...
                          {newsletter.status === "completed" &&
                            newsletter.completedAt && (
                              <p className="text-xs text-gray-500">
                                Complété le{" "}
                                {formatDateTime(newsletter.completedAt)}
                              </p>
                            )}
                        </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-full overflow-hidden">
            <div className="p-6 border-b border-gray-200 flex items-center justify-between">
              <div>
                ...
                <div className="flex items-center gap-4 text-sm text-gray-600 mt-2">
                  <span
                    className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusColor(viewingNewsletter.status)}`}
                  >
                  ...
                  <div className="flex items-center">
                    <Users className="h-4 w-4 mr-1" />
                    {viewingNewsletter.sentCount} /{" "}
                    {viewingNewsletter.totalRecipients} envoyés
                  </div>
                </div>
              </div>
              <button
            ...
            <div className="overflow-auto p-6" style={{ maxHeight: "70vh" }}>
                  ...
                  __html: (() => {
                    try {
                        ...
                        return delta.ops
                          .map((op: Record<string, unknown>) => {
                            if (typeof op.insert === "string") {
                              let text = op.insert;
                              if (op.attributes) {
                                const attrs = op.attributes as Record<
                                  string,
                                  unknown
                                >;
                                if (attrs.bold)
                                ...
                                if (attrs.link)
                                  text = `<a href="${attrs.link}" target="_blank" rel="noopener noreferrer" style="color: #1e40af;">${text}</a>`;
                                if (attrs.header)
                                  text = `<h${attrs.header} style="color: #1e40af; margin: 16px 0 8px 0;">${text}</h${attrs.header}>`;
                              }
                              return text.replace(/\n/g, "<br>");
                            }
                            return "";
                      ...
                      return viewingNewsletter.content
                        ? viewingNewsletter.content.replace(/\n/g, "<br>")
                        : "";
                    }
                    return viewingNewsletter.content || "";
                  })(),
              ...
              {viewingNewsletter.selectedPublicationIds &&
                viewingNewsletter.selectedPublicationIds.length > 0 && (
                  <div className="mt-6 pt-6 border-t border-gray-200">
                    <h4 className="text-sm font-semibold text-gray-900 mb-3">
                      Publications incluses
                    </h4>
                    <div className="text-sm text-gray-600">
                      {viewingNewsletter.selectedPublicationIds.length}{" "}
                      publication(s) incluse(s)
                    </div>
                  </div>
                )}
            </div>
