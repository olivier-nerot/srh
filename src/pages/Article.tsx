        ...
        .map(
          (op: {
            ...
            if (typeof op.insert === "string") {
              ...
              if (op.attributes) {
                ...
                if (op.attributes.link)
                  text = `<a href="${op.attributes.link}" target="_blank" rel="noopener noreferrer" class="text-blue-600 hover:text-blue-800 underline">${text}</a>`;
                if (op.attributes.header)
                  text = `<h${op.attributes.header} class="text-xl font-semibold mt-4 mb-2">${text}</h${op.attributes.header}>`;
              }
            ...
            if (typeof op.insert === "object" && op.insert.image) {
              ...
              return `<img src="${src}" alt="" class="max-w-full h-auto my-2 rounded" />`;
            }
            return "";
          },
        )
      ...
      if (data.success) {
        return (
          data.publications.find(
            (pub: Publication) => pub.id.toString() === id,
          ) || null
        );
      }
      return null;
  ...
  const fetchDocuments = async (
    ...
    try {
      const response = await fetch(
        `/api/files?action=list&ids=${attachmentIds.join(",")}`,
      );
      const data = await response.json();
            ...
            <div className="prose prose-lg max-w-none">
              <div
                ...
                dangerouslySetInnerHTML={{
                  __html: deltaToHtml(articleData.content),
                }}
              />
            </div>
          </div>
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <div className="text-sm text-gray-600 space-y-1">
                        ...
                        <p>
                          Taille: {(document.fileSize / 1024).toFixed(1)} KB
                        </p>
          ...
          <div className="flex items-center space-x-3 mb-4 sm:mb-0">
            ...
            <div>
              ...
              <p className="text-xs text-gray-600">
                Publié le{" "}
                {new Date(articleData.publishDate).toLocaleDateString("fr-FR")}
              </p>
            </div>
          </div>
